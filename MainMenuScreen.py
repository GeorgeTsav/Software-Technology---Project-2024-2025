...
class MainMenuScreen:
    def __init__(self, top=None, root=None, username=None):
        ...
        self.MyProfileButton.configure(command=self.open_my_profile)
    ...
    def open_my_profile(self):
       ...
       profile_screen = MyProfile.MyProfile(profile_window, username=self.username)
       profile_window.protocol("WM_DELETE_WINDOW", lambda: self.show_main_menu(profile_window))
    ...
    def show_main_menu(self, profile_window):
        profile_window.destroy()
        self.top.deiconify()
    ...
    def load_user_data(self):
        ...
        db.connect()
        cursor = db.execute_query("SELECT name, last_name, telephone FROM user WHERE username = %s", (self.username,))
        row = cursor.fetchone()
        cursor.close()
        db.close()
        
        if row:
            name, last_name, telephone = row
        else:
            name = last_name = telephone = "Unknown"
